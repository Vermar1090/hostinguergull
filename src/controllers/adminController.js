// src/controllers/adminController.js - VERSIÓN QUE SÍ INSERTA EN BD
...
const createProduct = async (req, res) => {
    try {
        ...
        const medium = req.file?.medium || null;
        ...
        // ✅ INSERTAR IMAGEN EN LA TABLA producto_imagenes
        if (imagen_url) {
            await db.query(
                'INSERT INTO producto_imagenes (producto_id, imagen_url, thumbnail, medium, orden, activo) VALUES (?, ?, ?, ?, ?, 1)',
                [productId, imagen_url, thumbnail, medium, 0]
            );
            console.log('✅ Imagen insertada en producto_imagenes para producto', productId);
        }

        emitToAll('product_created', { id: productId, nombre, imagen_url, precio_venta });
...
const updateProduct = async (req, res) => {
    try {
        ...
        // ✅ INSERTAR O ACTUALIZAR IMAGEN EN LA TABLA producto_imagenes
        if (imagen_url) {
            // Verificar si ya hay una imagen principal
            const [existingImages] = await db.query(
                'SELECT id FROM producto_imagenes WHERE producto_id = ? AND orden = 0 LIMIT 1',
                [id]
            );
            ...
            if (existingImages[0]) {
                ...
                await db.query(
                    'UPDATE producto_imagenes SET imagen_url = ?, thumbnail = ?, medium = ?, activo = 1 WHERE id = ?',
                    [imagen_url, thumbnail, medium, existingImages[0].id]
                );
                console.log('✅ Imagen actualizada en producto_imagenes para producto', id, 'ID:', existingImages[0].id);
            } else {
                // Insertar nueva imagen
                await db.query(
                    'INSERT INTO producto_imagenes (producto_id, imagen_url, thumbnail, medium, orden, activo) VALUES (?, ?, ?, ?, 0, 1)',
                    [id, imagen_url, thumbnail, medium]
                );
                console.log('✅ Nueva imagen insertada en producto_imagenes para producto', id);
            }
        }
...
const addProductImages = async (req, res) => {
    try {
        ...
        for (const img of images) {
            await db.query(
                'INSERT INTO producto_imagenes (producto_id, imagen_url, thumbnail, medium, orden, activo) VALUES (?, ?, ?, ?, ?, 1)',
                [id, img.original, img.thumbnail, img.medium]
            );
            console.log('✅ Imagen adicional insertada para producto', id);
        }

        emitToAll('product_images_added', { producto_id: id, count: images.length });

        res.json({ success: true, message: 'Imágenes agregadas correctamente', count: images.length });
    } catch (error) {
