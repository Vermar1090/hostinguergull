...
class Product {
   ...
   // backend/models/Product.js - CORREGIDO
...
static async getAll(filters = {}) {
    let query = `
        SELECT p.*,
               ...
               (SELECT COUNT(*) FROM producto_imagenes WHERE producto_id = p.id AND activo = 1) as total_imagenes
        FROM productos p
    ...
    // ✅ FILTRO es_oferta
    if (filters.es_oferta !== undefined && filters.es_oferta !== null) {
        const ofertaValue = parseInt(filters.es_oferta);
        if (!isNaN(ofertaValue)) {
            conditions.push('p.es_oferta = ?');
            params.push(ofertaValue);
        }
    }
    ...
    // ✅ Excluir productos con precio 0 (opcional)
    ...
    if (filters.sort_by) {
        const sortMap = {
            ...
            'popularidad': 'p.id DESC',  // ⚠️ Si no tienes columna 'orden', usa 'id'
            'nuevos': 'p.id DESC'
        };
    ...
    static async getById(id) {
        try {
            ...
            // Obtener imágenes del producto (solo activas)
            const [images] = await db.execute(
                'SELECT * FROM producto_imagenes WHERE producto_id = ? AND activo = 1 ORDER BY orden',
                [id]
            );
            product.imagenes = images;
            
            // Obtener opciones de personalización (solo activas)
            ...
            // Obtener valores de cada opción (solo activos)
    ...
    // Obtener productos por puntos canjeables
    ...
    // === GESTIÓN DE IMÁGENES ===
    static async addImage(producto_id, imagen_url, orden = 0) {
        // Verificar si existe la columna activo, si no existe, no la incluimos
        try {
            const query = `
                INSERT INTO producto_imagenes (producto_id, imagen_url, orden)
                VALUES (?, ?, ?)
            `;
            const [result] = await db.execute(query, [producto_id, imagen_url, orden]);
            return result.insertId;
        } catch (error) {
            // Si hay error por columna activo, intentamos sin ella
    ...
    static async addOption(producto_id, data) {
        ...
        const query = `
            INSERT INTO opciones_personalizacion (producto_id, nombre_grupo, tipo, obligatorio, max_selecciones, orden, activo)
            VALUES (?, ?, ?, ?, ?, ?, 1)
        `;
    ...
    static async addOptionValue(opcion_grupo_id, data) {
        ...
        const query = `
            INSERT INTO opciones_valores (opcion_grupo_id, valor, precio_extra, stock_disponible, orden, activo)
            VALUES (?, ?, ?, ?, ?, 1)
        `;
    ...
    static async addExtra(producto_id, data) {
        ...
        const query = `
            INSERT INTO extras_producto (producto_id, extra_nombre, precio_extra, max_por_pedido, activo)
            VALUES (?, ?, ?, ?, 1)
        `;
    ...
    // Obtener historial de kardex de un producto
